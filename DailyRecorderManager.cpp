...
{
	fileInit();
	MyWindow::MyWindow(640, 680, EW_SHOWCONSOLE | EX_NOCLOSE);//初始化窗口
...
void DailyRecorderManager::fileInit()
{
	...
	do
	{
		...
		string IMAGETOSELECT = "./DailyRecorderImages/ImagesToSelect/" + (string)fileinfo.name;//创建string类型文件路径
...
int DailyRecorderManager::recordersSelectAndRecordersDeletePage(int mode)
{
	...
	while (1)
	{
		...
		default:
		{
			if (mode == 1)op = showRecorder(op - 1);//显示
			if (mode == 2)op = delete_(op - 1);		//删除
			break;
		}
		}
	}
