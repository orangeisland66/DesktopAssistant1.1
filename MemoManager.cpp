...
{
	fileInit();													//文件初始化
	MyWindow::MyWindow(640, 680, EW_SHOWCONSOLE | EX_NOCLOSE);	//创建窗口
	...
	setbkmode(TRANSPARENT);					//将背景模式设置为透明
	//加载图片
...
void MemoManager::operator()()
{
	...
	while (true)
	{
		...
		case 2:goto flag;//以后少用goto，老师原谅我~
		}
	}
flag:
	cout << "正在退出……\n";
...
//将string时间转化为int类型存储
bool MemoManager::transform1(string YMD,int *Y_M_D)
{
	...
	ss << temp;
	ss >> Y_M_D[count];
	//判断日期是否合法，避免2023/12/32被录入
	if (Y_M_D[0] > 2027)return false;
...
void MemoManager::reminder_and_ddlShow()
{
	...
	for (vector<Subject>::iterator it = subjectArray.begin(); it != subjectArray.end(); it++)
	{
		...
		if ((*it).is_ddl())
		{
				int dayRemain = calculateDay((*it).getYear(), (*it).getMonth(), (*it).getDay());//判断ddl是否已经过去
...
int MemoManager::deleteSubject(int i)
{
	...
	int flag = MessageBox(MyWindow::getHWND(), "你确定要删除该课程吗？", "提示", MB_YESNO);//弹出提示框，鼠标按yes返回6，no返回7
	if (flag == 6)
	{
...
int MemoManager::add_ddl(int i)
{
	...
	if (!cinCheck<bool>(4, 0, dateInput)|| !transform1(dateInput, year_month_date))
	{
		MessageBox(MyWindow::getHWND(), "输入格式错误，添加失败！", "提示", MB_OK);
		return 7;
	}

	//判断时间是否合法，以及是否已经添加了ddl
	SystemTime::getSystemTime();
	bool flag = true;
...
bool MemoManager::isEsc()
{
	if (MyWindow::hasMsg())
	{
		...
		switch (msg.message)
		{
		case WM_KEYDOWN:				//按下了键盘
			if (msg.vkcode == VK_ESCAPE)//按下了Esc健
			{
				cleardevice();
				return true;
			}//esc退出主界面
...
int MemoManager::calculateDay(int year,int month,int day)
{
	...
	struct tm t2 = { 0 };
	double seconds;
	t1.tm_year = SystemTime::getYear() - 1900; 
	t1.tm_mon = SystemTime::getMonth() - 1;
	t1.tm_mday = SystemTime::getDay();				//现在时间
	t2.tm_year = year - 1900;
	t2.tm_mon = month - 1; 
	t2.tm_mday = day;								//ddl时间
	seconds = difftime(mktime(&t2), mktime(&t1));	//转换结构体为time_t,利用difftime,计算时间差  
	if (seconds < 0)return -1;
	return (int)(seconds / 86400);					//最后输出时间,因为一天有86400秒
}
//析构函数
MemoManager::~MemoManager()
{
